function arx_KB(N,sigma,lambda) 
%
...
lambdaPatrat = 1 ; 
numerArgumente = 2;
Rv = ones(numerArgumente) ; % Rv>0 (pagina 91)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NEW STUFF %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
v = version('-release'); % get version of Matlab
if (strcmp(v, '2014b') || strcmp(v, '2015a')) 
    [theta,ypred] = rarx(D,[1 1 1],'kf',Rv) ; %Se modifica aici
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% NEW STUFF %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%2 Initializare
alpha = 1; %Se alege alpha
Priv = alpha*eye(length(theta)) ; % in cazul in care theta0 se alege arbritar
                             % daca e calculat se alege asa cum e scris in codul necomentat
Priv = Rn^(-1) ; 
Rv =ones(size(Priv)) ;
...
for k = 1 : N   
   ...
   tzeta = Priv*tau ;
   gain = tzeta/(lambdaPatrat + phi'*tzeta);
   Priv = Priv + Rv - gain*phi'*Priv ;
   theta = theta + gain*eps(k) ;
