...
%
% ISLAB_4A   Module that estimates an ARX model by using MVI. 
%
...
% Alegere metoda de estimare
disp('Alegeti metoda:');
disp('1: MCMMP');
disp('2: MVI - nefiltrat');
disp('3: MVI - partial filtrat (primele na)');
disp('4: MVI - partial filtrat (ultimele nb)');
disp('5: MVI - total filtrat');

metoda = input('Alegeti metoda de estimare:', 's');
...
for na=1:Na
   for nb=1:Nb
      if ((na>1) || (nb>1)) 
         ...
         % Switch intre metode
         switch (metoda)
             case '1' % MCMMP - ISLAB_5A
                  Mid = arx(Did,[na-1 nb-1 nk]); % Model estimation.
                  
             case '2' % MVI - nefiltrat - ISLAB_5B
                 Mid = iv4(Did,[na nb nk]); % Model estimation.
                
             case '3' % MVI - partial filtrat - na sunt filtrate si nb sunt nefiltrate
                 model_aux = arx(Did, [na nb 1]); 
                 %se produce un model estimat folosind MCMMP
                 uf = filter (model_aux.B, model_aux.A, Did.u); 
                 %filtrarea datelor din Did.u cu filtrul descris de
                 %vectorii A si B si creaza datele filtrate uf
                 %uf = uf/norm(uf); %calculare norma L2 si normalizare
                 uf = uf./sqrt(ones(N,1)*sum(uf.*uf)/N) ; 
                 %normalizarea variabilelor intrumentelor filtrate
                 Did.u((na+1):(na+nb)) = Did.u(1:nb); 
                 Did.u(1:na) = uf(1:na); 
                 %De la 1:na sunt filtrate si de la na+1:na+nb sunt nefiltrate
                 Mid = iv4(Did,[na nb nk] ) ;% Model estimation.

             case '4' % MVI - partial filtrat - na sunt nefiltrate si nb sunt filtrate
                 model_aux = arx(Did, [na nb 1]); 
                 %se produce un model estimat folosind MCMMP
                 uf = filter (model_aux.B, model_aux.A, Did.u); 
                 %filtrarea datelor din Did.u cu filtrul descris de
                 %vectorii A si B si creaza datele filtrate uf
                 %uf = uf/norm(uf); %calculare norma L2 si normalizare
                 uf = uf./sqrt(ones(N,1)*sum(uf.*uf)/N) ; %normalizarea variabilelor intrumentelor filtrate
                 Did.u((na+1):(na+nb)) = uf(1:nb); 
                 %Se inlocuiesc ultimele nb u-uri cu primele nb uf-uri in Did.u
                 Mid = iv4(Did,[na nb nk] ) ;% Model estimation.
                
             case '5' % MVI - filtrat total
                 model_aux = arx(Did, [na nb 1]);
                 uf = filter (model_aux.B, model_aux.A, Did.u);
                 %uf = uf/norm(uf); %calculare norma L2 si normalizare
                 uf = uf./sqrt(ones(N,1)*sum(uf.*uf)/N) ; %normalizarea variabilelor intrumentelor filtrate
                 Did.u(1:na+nb) = uf(1:na+nb); %Se inlocuiesc toate u-urile cu uf-uri in Did.u
                 Mid = iv4(Did,[na nb nk]) ;% Model estimation.
