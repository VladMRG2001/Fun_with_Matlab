function [a,b,lambda,magi,phii,mag,phi,f] = ISLAB_4E(at,bt,K,N,nr) 
%
% ISLAB_4E   Module that estimates the parameters of an 
...
%
% Explanation:	The parameters of an ARX[1,1] model are 
...
a = roots(at) ; 
if (abs(a(1))>=1)
   war_err(['<ISLAB_4E>: Model unstable. ' ...
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% u = filter(1,[1 p],u) ; % u nu este filtrat la 4E 
u = u./sqrt(ones(N,1)*sum(u.*u)/N) ; 
% 
% Generating the nr realizations
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
y = filter(bt,at,u) + e ; % eroarea nu este filtrata
% 
% Estimating the OE parameters 
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
a = zeros(nr, 1) ; 
b = zeros(nr, 1) ;
lambda = zeros(nr, 1) ;
...
for p=1:nr
   ...
   R = [ru(1) Rtemp ; ... 
        Rtemp'  ry(1)];  		 	    % Main matrix.  
   r = [-ry(2) ; ryu(2)] % Free vector. 
   r = R*r / (ru(1) * ry(1) - ryu(1)^2); 			    % Coefficients. 
   a(p, :) = r(1:1) ; 
   b(p, :) = r(2:2) ; 
   e = [y(:,p) ... 
         [0 ; y(1:(N-1),p)] ... 
        -[0 ; u(1:(N-1),p)]] * [1 ; r] ;% Noise estimation. 
   lambda(p) = norm(e)/sqrt(N-2) - 1/5 ;% Standard deviation.  
% 
...
figure(FIG),clf ; 
   ...
   subplot(211) ; 
      ...
      title(['Estimating an OE[1,1] model ' ... 
