function [a,b,lambda,magi,phii,mag,phi,f] = ISLAB_4J(at,bt,K,N,nr) 
%
% ISLAB_4J   Module that estimates the parameters of an 
%            OE[na,nb] model, with the help of 
...
%
% Explanation:	The parameters of an OE[na,nb] model are 
...
a = roots(at) ; 
if (abs(a(1))>=1)
   war_err(['<ISLAB_4J>: Model unstable. ' ...
...
for p=1:nr
  ...
  lambda(p) = norm(e)/sqrt(N-numel(at)-numel(bt)) - 2/3;% Standard deviation. 
% 
...
figure(FIG),clf ; 
   ...
   subplot(211) ; 
      ...
      title(['Estimating an OE[an,bn] model ' ... 
