function [Mid,Did,Dva] = ISLAB_6B(B,C,D,F,nk,N,sigma,lambda) 
%
% ISLAB_5A   Module that estimates an BJ model by using MPEM. 
%
% Inputs:	    B = ([1 0.5], by default)
%        	    C = ([1 -1 0.2], by default)
%               D = ([1 1.5 0.7], by default)
%               F = ([1 -1.5 0.7], by default) == A pt Armax
...
%
% Explanation:	Data generated by an BJ process are employed 
%               to identify an BJ model with the help of 
...
%F este egal cu A, astfel putem inlocui na cu nf peste tot
      ...
      if (( (nf>1) || (nb>1) || (nc>1) || (nd>1)))
         if ((nb<2) && (nc<2))	&& (nd<2)	% Model estimation. 
            ...
            Mid.nk = nk ;
         elseif (nb<2)
            Mid = bj(Did,[0 nc-1 nd-1 nf-1 nk]) ; %Se schimba din armax in bj 
            Mid.b = 0 ; 
            Mid.nk = nk ;
         else
